...
const post = async (body: unknown) => POST(new NextRequest(`${INTERNAL_ORIGIN}/mcp`, { method: "POST", body: JSON.stringify(body), headers: { "content-type": "application/json" } }));
...
it("lists the four agent-marketplace categories and links them to this origin", async () => {
  ...
  expect(agents.every((agent: { deployUrl: string }) => agent.deployUrl.startsWith(`${PUBLIC_ORIGIN}/deploy/`))).toBe(true);
...
it("says a hire needs the owner's own passkey signature, and never offers to do it", async () => {
  ...
  expect(result.deployUrl).toBe(`${PUBLIC_ORIGIN}/deploy/grid`);
...
it("answers malformed and unsupported JSON-RPC without a 500", async () => {
  const parse = await POST(new NextRequest(`${INTERNAL_ORIGIN}/mcp`, { method: "POST", body: "not json", headers: { "content-type": "application/json" } }));
...
it("GET is a live descriptor a health probe can classify, not an error", async () => {
  const response = await GET(new NextRequest(`${INTERNAL_ORIGIN}/mcp`));
  expect(response.status).toBe(200);
  const body = await response.json();
  expect(body.endpoint).toBe(`${PUBLIC_ORIGIN}/mcp`);
