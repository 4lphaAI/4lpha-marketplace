...
const SCOPED_RECORD_PREFIXES = ["4lpha:grid-hire-choices:v1", "4lpha:lp-hire:v1:", "4lpha:lending-hire:v1:"];
...
function isScopedKey(k: string): boolean {
  return POINTERS.includes(k) || SCOPED_RECORD_PREFIXES.some((prefix) => k === prefix || k.startsWith(prefix));
}
...
export function accountHireStorage(storage: Storage | undefined, owner: string | undefined): Storage {
  ...
  const map = (k: string) => isScopedKey(k) ? scoped(k, owner ?? "unselected") : k;
  return {
    ...
    getItem: (k) => {
      ...
      return value === null && owner && isScopedKey(k) && storage.getItem(INITIALIZED) === null ? storage.getItem(k) : value;
    },
    setItem: (k, value) => storage.setItem(storage.getItem(INITIALIZED) === null ? k : map(k), value),
    removeItem: (k) => {
      storage.removeItem(map(k));
      if (owner && isScopedKey(k) && storage.getItem(INITIALIZED) === null) storage.removeItem(k);
    },
  };
}
