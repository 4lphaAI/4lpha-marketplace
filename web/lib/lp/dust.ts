...
function amount(wei: bigint, decimals: number): string {
  const text = formatAtomic(wei.toString(), decimals, 3);
  if (text === null) return wei.toString();
  return wei > 0n && text === "0" ? "<0.001" : text;
}
...
export function dustMetric(input: {
  ...
  return {
    value: `${amount(read.token0Wei, input.decimals0)} ${input.symbol0} / ${amount(read.token1Wei, input.decimals1)} ${input.symbol1}`,
    reason: null,
  };
}
