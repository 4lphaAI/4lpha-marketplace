...
export type CoverageReason = "none" | "capacity" | "dependency" | "stale" | "unpriced" | "unreadable" | "identity-conflict" | "unsupported-profile" | "zero-basis" | "missing-mark" | "held" | "declared";
...
export type AccountPortfolioDeps = {
  ...
  readonly lp?: { readonly store: LpSequenceStore; readonly observations: LpObservationStore; readonly workerIntervalMs: number };
  /**
  ...
  for (const agent of agents) {
    ...
    if (agent.httpRuntimeProfile === "lp-v1" && isLive(agent)) {
      if (lpUnavailable) {
        ...
        agentViews.push({ id: agent.id, status: agent.status, httpRuntimeProfile: agent.httpRuntimeProfile, walletAddress: agent.walletAddress, attention: "partial-data", holdings: { method: "sellable-lp-exit-v1", state: "unavailable", reason: "dependency", valueUsdMicros: null, venusReference: null, held: false }, pnl: { method: "gross-lp-mark-to-declared-basis-v1", coverage: "unavailable", reason: "dependency", eligibleBasisNativeWei: null, markNativeWei: null, pnlNativeWei: null, pnlUsdMicros: null, pnlBps: null, basisSources: [], excluded: [...EXCLUDED] } });
        continue;
      }
      ...
      const full = missing === 0 && zeroBasis === 0;
      if (missing > 0) deployedComplete = false;
      if (!full && rows.length > 0) pnlComplete = false;
      if (full && basis > 0n) { eligibleBasis += basis; aggregatePnl += pnl; }
      const valueUsd = bnbPrice === null || missing > 0 ? null : signedNativeUsd(mark, bnbPrice.micros);
      const pnlUsd = bnbPrice === null || basis === 0n ? null : signedNativeUsd(pnl, bnbPrice.micros);
      const pnlCoverage = rows.length === 0 ? "full" : basis === 0n ? "unavailable" : full ? "full" : "partial";
      const publishPnl = pnlCoverage === "full" && basis > 0n;
      agentViews.push({ id: agent.id, status: agent.status, httpRuntimeProfile: agent.httpRuntimeProfile, walletAddress: agent.walletAddress, attention: agent.status === "paused" ? "paused" : missing > 0 || zeroBasis > 0 ? "partial-data" : "none", holdings: { method: "sellable-lp-exit-v1", state: rows.length === 0 ? "empty" : missing > 0 ? "partial" : "complete", reason: missing > 0 ? "missing-mark" : held ? "held" : "none", valueUsdMicros: valueUsd?.toString() ?? null, venusReference: null, held }, pnl: { method: "gross-lp-mark-to-declared-basis-v1", coverage: pnlCoverage, reason: missing > 0 ? "missing-mark" : zeroBasis > 0 ? "zero-basis" : held ? "held" : "none", eligibleBasisNativeWei: publishPnl ? basis.toString() : null, markNativeWei: publishPnl ? mark.toString() : null, pnlNativeWei: publishPnl ? pnl.toString() : null, pnlUsdMicros: publishPnl ? pnlUsd?.toString() ?? null : null, pnlBps: publishPnl ? ((pnl * 10_000n) / basis).toString() : null, basisSources: publishPnl ? [...sources].sort() : [], excluded: [...EXCLUDED] } });
    } else {
      if (isLive(agent)) pnlComplete = false;
      agentViews.push({ id: agent.id, status: agent.status, httpRuntimeProfile: agent.httpRuntimeProfile, walletAddress: agent.walletAddress, attention: agent.status === "paused" ? "paused" : agent.status === "provisioning" ? "provisioning" : "none", holdings: { method: agent.httpRuntimeProfile === "venus-v1" ? "owner-wide-venus-stored-net-v1" : "none", state: agent.status === "provisioning" ? "partial" : "empty", reason: agent.status === "provisioning" ? "dependency" : "none", valueUsdMicros: null, venusReference: agent.httpRuntimeProfile === "venus-v1" ? "owner-wide" : null, held: false }, pnl: { method: "none", coverage: isLive(agent) ? "unsupported" : "unavailable", reason: isLive(agent) ? "unsupported-profile" : "none", eligibleBasisNativeWei: null, markNativeWei: null, pnlNativeWei: null, pnlUsdMicros: null, pnlBps: null, basisSources: [], excluded: [] } });
    }
  }
